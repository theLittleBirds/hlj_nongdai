                            ...
                            <div id="search-panel">
                                <div class="panel-body">                                   
                                    <div class="form-inline">
                                        ...
                                        <button class="btn btn-primary" type="button" onclick="productAdd()"><i class="fa fa-search"></i> 新增商品
                                        </button>
									</div>
                                </div>
                            </div>
...
<script type="text/javascript">
 $(document).ready(function(e){
    $('#tableid').bootstrapTable({
			method: 'post', 
			url: "/njProduct/asyncLoanRiskPostList",
			dataType: "json",
			cache: false,
...
function actionFormatter(value, row, index) {
	var html = "<button class='btn btn-primary' type='button' onclick='productEdit(\""+row.id+"\",this)'><i class='fa fa-search'></i>编辑</button>"
					+"<button class='btn btn-primary' type='button' onclick='productDelete(\""+row.id+"\",this)'><i class='fa fa-search'></i>删除</button>"
    return html;
}
...
function productEdit(id){
	window.location.href = "/njProduct/toNjProductSaveOrEdit?id="+id;
}

//添加 
function productAdd(){
	window.location.href = "/njProduct/toNjProductSaveOrEdit";
}
            ...
            if(result){
            	$.ajax({
	        	    ...
	        	    success:function(data,textStatus,jqXHR){
	        	        ...
	        	        if(data){
	        	        	parent.window.bootbox.alert("商品删除成功！");
	        	        	window.location.reload();
	        	        }else{
	        	        	parent.window.bootbox.alert("商品删除失败！");
	        	        	window.location.reload();
	        	        }
	        	    }
	        	}) 
