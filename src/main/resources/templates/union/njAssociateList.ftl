                                    ...
                                    <div class="form-inline">
										...
										<div class="form-group">
									      ...
									      <select id="status" class="form-control">
			                                 ...
			                                 <option value="2">已付定金，商户待接单</option>
			                                 <option value="3">商户已接单，待付尾款</option>
			                                 <option value="4">商户已发货</option>
			                                 <option value="5">已完成</option>
			                               </select>
									    </div>
									    
									    <div class="form-group">
									      <label class="control-label" for="merName">商家名称:</label>
...
$(document).ready(function(e){
    $('#tableid').bootstrapTable({
			method: 'post', 
			url: "/njAssociate/asyncLoanRiskPostList",
			dataType: "json",
			cache: false,
...
function actionFormatter(value, row, index) {
	...
	if(row.status == 1 && row.isDeleted == 0){
		html = "<button class='btn btn-primary' type='button' onclick='firstPay(\""+row.id+"\")'><i class='fa fa-search'></i>线上付定金</button><button class='btn btn-primary' type='button' onclick='lastPay(\""+row.intoPieceId+"\",\""+row.id+"\",\"fdj\")'><i class='fa fa-search'></i>线下付定金</button>";
	}else if(row.status == 3 && row.isDeleted == 0){
		html = "<button class='btn btn-primary' type='button' onclick='firstPay(\""+row.id+"\")'><i class='fa fa-search'></i>线上付尾款</button><button class='btn btn-primary' type='button' onclick='lastPay(\""+row.intoPieceId+"\",\""+row.id+"\",\"fwk\")'><i class='fa fa-search'></i>线下付尾款</button>";
	}else{
...
function firstPay(id){
	parent.window.bootbox.alert("该功能暂缓开通");
}

function lastPay(intoPieceId,id,type){
	var url = "/picture/showlist?intoPieceId="+intoPieceId+"&type="+type+"&id="+id;
	window.location.href= url;	
}

function picture(intoPieceId,id){
	var url = "/picture/showlist?intoPieceId="+intoPieceId+"&id="+id;
	window.location.href= url;
}
