...
function checkForm(){	
	...
	if(paymentDate == ''){
		parent.window.bootbox.alert("请选择打款日期");
		return false;
	}
	...
	$.ajax({
		type: "POST",
		dataType: "json",
		url: "/union/underlinepaysave",
		data: {"intoPieceId":intoPieceId,"paymentDate":paymentDate},
	    error: function(request) {	
	    },
	    success: function(data) {
	    	parent.window.bootbox.alert(data.msg);
	    	if(data.code == 200){
	    		window.location.href="/union/waitpay";
	    	}
	    }
	});
	return false;
	...
	var observer = {
			  ...
			  complete:function(res){
				  $.ajax({
						type: "POST",
						url: "/picture/save",
						data: {"intoPieceId":$("#intoPieceId").val(),"fileType": 1,"hashKey":res.hash},
					    error: function(request) {	
					    },
					    success: function(data) {						    	
					    	sum = sum + 1;
...
function refreshMedia(){
	$.ajax({
		type: "POST",
		url: "/picture/list",
		data: {"intoPieceId":$("#intoPieceId").val(),"type":"1","fileType":1},
	    error: function(request) {	
