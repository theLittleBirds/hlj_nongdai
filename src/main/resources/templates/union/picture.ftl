                            ...
                            <div class="col-md-12 text-center">
                            	<#if (type!'无') == 'fdj'>
	                            	<button class="btn btn-w-m btn-success" type="button" onclick="operateOrder()">
		                                <i class="ace-icon fa fa-check bigger-110"></i> 付定金
		                            </button> 
	                            </#if>
	                            <#if (type!'无') == 'fwk'>
		                            <button class="btn btn-w-m btn-success" type="button" onclick="operateOrder()">
		                                <i class="ace-icon fa fa-check bigger-110"></i> 付尾款
		                            </button> 
	                            </#if>
	...
	var observer = {
			  ...
			  complete:function(res){
				  $.ajax({
						type: "POST",
						url: "/picture/save",
						data: {"intoPieceId":$("#intoPieceId").val(),"type":"2","orderId":$("#id").val(),"orderType":"2","fileType": fileType,"hashKey":res.hash},
					    error: function(request) {	
					    },
					    success: function(data) {
					    	//记录成功上传图片的次数（进入页面初始化为0）
...
function refreshMedia(){
	...
	$.ajax({
		type: "POST",
		url: "/picture/merList",
		data: {"intoPieceId":$("#intoPieceId").val(),"type":"2","orderId":$("#id").val(),"orderType":"2","fileType":fileType},
	    error: function(request) {	
...
function operateOrder(){
	...
	//付定金
	if(type == 'fdj' && picNum > 0){
		//检查上传过付定金类型图片，且本次成功上传过的允许提交
		$.ajax({
		    url:'/picture/checkList',
		    type:'POST', 
		    async:true,
		    data:{"intoPieceId":$("#intoPieceId").val(),"type":"2","fileType":"3","orderId":$("#id").val(),"orderType":"2"},
		    dataType:'json', 
		    success:function(data){
		    	if(data.code==200){
		    		//上传定金回单成功的允许更改订单信息
		    		$.ajax({
		    		    url:'/njAssociate/njAssociateSave',
		    		    type:'POST', 
		    		    async:true,
		    		    data:{
		    		    	id: id
		    		...
		    		parent.window.bootbox.alert("请上传定金回单");
		    	}
		    }
		})
	}
	//付尾款
	if(type == 'fwk' && picNum > 0){
		//检查上传过付尾款类型图片，且本次成功上传过的允许提交
		$.ajax({
		    url:'/picture/checkList',
		    type:'POST', 
		    async:true,
		    data:{"intoPieceId":$("#intoPieceId").val(),"type":"2","fileType":"4","orderId":$("#id").val(),"orderType":"2"},
		    dataType:'json', 
		    success:function(data){
		    	if(data.code==200){
		    		//上传尾款回单成功的允许更改订单信息
		    		$.ajax({
					    url:'/njAssociate/njAssociateFinalPaySave',
					    type:'POST', 
					    async:true,
					    data:{
					    	id: id
		    		...
		    		parent.window.bootbox.alert("请上传尾款回单");
		    	}
		    }
		})		
	}
