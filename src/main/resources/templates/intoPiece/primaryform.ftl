        ...
        <div class="col-sm-12">
            <div class="ibox float-e-margins">
                <div class="ibox-content clearfix">
                    <form class="form-horizontal col-lg-8 m-t" id="primaryForm" onsubmit="return checkForm()"
                          method="post">
                        ...
                        <div class="form-group">
                             ...
                             <div class="col-sm-3">
                                 <input type="text" id="capital" value="${capital}" name="capital"  onchange="javascript:calculation('1');calculatePrice(this);" 
                                         class="form-control"/>                                 
                             </div>

                             <label class="col-sm-2 control-label">借款期限(月)：</label>
                             <div class="col-sm-3">
                                 <input type="number" id="term" value="${term}" name="term" onchange="calculation('2')" 
                                        class="form-control">
                             </div>
                        </div>
                             ...
                             <div class="col-sm-3">
                                 <select  id="product" name="product" class="form-control" onchange="calculation('3')" >
                                 	...
                                 	<#list productList as one>   
                                 	  <#if one.productId == product>
                					    <option value="${one.productId}" selected="selected">${one.name}</option>
                					  <#else>    
                        ...
                        <div class="form-group">
                             ...
                             <label class="col-sm-2 control-label">客户资料核实情况：</label>
                             <div class="col-sm-3">
                                  <select  id="memberInfoChecking"  name="memberInfoChecking" class="form-control">  
                                  	${DATA_VERIFY}
                                  </select>                     
                             </div>
                        </div>
                       ...
                       <!--  <div class="form-group">
                             ...
                             <div class="col-sm-8">
                                 <textarea id="familyRecordSummary" name="familyRecordSummary"
                                         class="form-control"></textarea>                              
                             </div>
                        </div>                       
                        
                        <div class="form-group">
                             <label class="col-sm-2 control-label">资产负债核情况：</label>
                             <div class="col-sm-8">
                                 <textarea id="assetChecking" name="assetChecking"
                                         class="form-control"></textarea>                              
                             </div>
                        </div>
                        
                        <div class="form-group">
                             <label class="col-sm-2 control-label">经营情况：</label>
                             <div class="col-sm-8">
                                 <textarea id="asset" name="asset"
                                         class="form-control"></textarea>                              
                             </div>
                        </div> -->
                        </div>
                        ...
                        <div class="form-group">
                             <label class="col-sm-2 control-label">总站意见：</label>
                             <div class="col-sm-8">
	...
	$.ajax({
	    ...
	    success: function(data) {
	    	if(data.code == 200){
	    		...
	    		if(history.length == 0){//没查到历史记录，自动调用评分
	    			getscore();
	    			return;
	    		}
...
function checkForm(){
	...
	if(nodeType == 1){
		...
		if(memberInfoChecking == ""){
			parent.window.bootbox.alert("客户资料核实情况必填");
			return false;
		}		
		if(opinion == ""){
			parent.window.bootbox.alert("总站意见必填");
			return false;
		}		
	}
	if(nextNodeId == ""){
...
//		{"capital" : capital , "id" : intoPieceId , "product" : jinrongPro , "term" : term , "type" : appType ,"recieveNongZi" : recieveNongZi , "recieveCash" : recieveCash , "productListInfo" : JSON.stringify(productArray) }
		$.ajax({
		    ...
		    success:function(data){
		    	if(data.code==200){
		    		window.location.href="/examine/primary";		    		
		    	}else{
		    		parent.bootbox.alert(data.msg);
		    	}
		    }
