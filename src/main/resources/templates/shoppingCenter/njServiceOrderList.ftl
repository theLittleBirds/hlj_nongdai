                            ...
                            <div id="search-panel">
                                <div class="panel-body">                                   
                                    <div class="form-inline">
                                        ...
                                        <div class="form-group">
                                            <label class="control-label" for="memberName">用户名称:</label>
...
<script type="text/javascript">
 $(document).ready(function(e){
    $('#tableid').bootstrapTable({
			method: 'post', 
			url: "/njServiceOrder/njOrderList",
			dataType: "json",
			cache: false,
...
function actionFormatter(value, row, index) {
    var html;
    if(row.status == "1"){
        html = "<button class='btn btn-primary' type='button' onclick='productEdit(\""+row.orderId+"\",this)'><i class='fa fa-search'></i>查看凭证</button>";
	}else{
        html = "<button class='btn btn-primary' type='button' onclick='productEdit(\""+row.orderId+"\",this)'><i class='fa fa-search'></i>详情</button>";
    }
